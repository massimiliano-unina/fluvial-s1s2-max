...
import os
from util import unsupervised_train_generator_2, unsupervised_val_generator_2,train_generator_3, val_generator_3, train_ml_models
...
def l_iou2(y_true, y_pred): 
    return l_iou(y_true, y_pred) + categorical_crossentropy(y_true,y_pred)
...
size = 128
size_t = 144 # 128 #
n_epochs = 5 # 5# 10# 1 #5 # 1 ## 20
n_batch = 32
LR_MAX = 0.05 # 0.05 # 0.005 # 0.02 # 
comb = ["Tri"] # ["B8"]#  ["VV", "VH","VVaVH","Tri"] # ["VV", "VH","VVaVH", "Tri"] #  ["VV", "VH", , "VVaVHaSum","VVaVHaRatio", "VVaVHaDiff","Total"]#"Ratio",  #["VVaVH"]#Ratio"]# ["VVaVH"]##
...
for date_alb in range(1):#4):
    ...
    #  
    files_test = [r"D:\fiumi unsupervised\Training_Set_Unsupervised\test_intradate_beta"+date_3+".npz"]#, "test_intradate_gamma.npz", "test_intradate_beta.npz"]
    dic_name_inputs = {r"D:\fiumi unsupervised\Training_Set_Unsupervised\test_intradate_beta"+date_3+".npz":"B11_8_4_", "test_intradate_gamma.npz":"gamma_", "test_intradate_a.npz":"beta_"}
    folder_trains = {r"D:\fiumi unsupervised\Training_Set_Unsupervised\test_intradate_beta"+date_3+".npz":r"D:\fiumi unsupervised\Training_Set_Unsupervised\\", "test_intradate_gamma.npz":r"D:\Albufera_2019_processed\Gamma2\\", "test_intradate_a.npz":r"D:\Albufera_2019_processed\Beta2\\"}
...
########## file test 
        ...
        for combinations in comb:
            ...
            for k_back in range(len(backs)):
                ...
                networks = ["Fractal_Net"] # ["Unet"]#  ["DenseUNet"]#["shallow_CNN"]# ["Fractal_Net", "FractalNet","shallow_CNN" , "Unet","Linknet","SegNet","FPN"] # ["SegNet","FPN"]#   "NestNet" "shallow_CNN"] #  ["SVM", "RF", "GBC", 
        ...
        #                bn1 = BatchNormalization()(l1)
                    ...
                    #    out= Conv2D(1, kernel_size=k_3, activation='relu', padding='same', kernel_initializer='he_normal',name="nothing")(out1)
                        ...
                        conv9b = Conv2D(f, kernel_size=3, activation=active, padding= 'same', kernel_initializer = keras.initializers.glorot_normal(seed=None))(conv9b)
                        conv9b = BatchNormalization()(conv9b)
                        outputs = Conv2D(N, kernel_size=1, activation=active, padding= 'same', kernel_initializer = keras.initializers.glorot_normal(seed=None), name="mae_output")(conv9b)
...
# print(dic)
            ...
            # ######## TRAINING ############
                    if k_mod == "SVM" or k_mod == "RF" or k_mod == "GBC" :
                        ...
                        losses = {"segmentation": l_iou2, "mae_output": l_mae} # categorical_crossentropy, "mae_output": l_mae}
                        lossWeights = {"segmentation": 1.0, "mae_output":  0.25}
                        # lossWeights = {"segmentation": 0.0, "mae_output":  1.0}
                        model.compile(loss = losses, loss_weights = lossWeights, metrics = [categorical_crossentropy,l_mae], optimizer = Adamax) # 
            ...
            #            
                        ...
                        model.fit_generator(unsupervised_train_generator_2(train_val_p2, number_train, n_batch,folder_train, size,N, combinations), validation_data = unsupervised_val_generator_2(train_val_p2, number_val, n_batch,folder_train, size,N, combinations), validation_steps = val_pe, steps_per_epoch = s_p_e, epochs = n_epochs,  callbacks = callbacks, class_weight=[[1.0,100.0, 200.0],[1.0]])
                        ...
                        model.save_weights( date_2 + name_inputs +combinations + "_Supervised_Post3" + BACKBONE + "_" + name_model + "_model_wIoU"+ str(size) +".h5")
        ...
        ############END TRAINING#############           
                    ...
                    if k_mod == "SVM" or k_mod == "RF" or k_mod == "GBC": 
                        ...
                        # "C:\Users\massi\Downloads\segmentation_models-master\1B11_8_4_Tri_Supervisedmobilenetv2_Fractal_Net_New3_model_wIoU128.h5"
                        # model.load_weights( name_inputs +combinations + "_" + BACKBONE + "_" + name_model + "_temporary_model_IoU"+ str(size) +".h5")
                        model.load_weights(date_2 + name_inputs +combinations + "_Supervised_Post3" + BACKBONE + "_" + name_model + "_model_wIoU"+ str(size) +".h5")
    ...
    # #########TESTING ############################
            ...
            #                 opt_test = Y1.astype('float32')
                            ...
                            x_val_1 = np.ndarray(shape=(1, mm_p*128, nn_p*128, 3), dtype='float32')
                            ...
                            pred_val_x = np.ndarray(shape=(1, mm_p*128, nn_p*128, 3), dtype='float32')
                            k = 1
                            for m_pred in range(0, mm_p):
                                for n_pred in range(0,nn_p):
                                    ...
                                    print(x_val_1[:,(m_pred1-1)*128:m_pred1*128, (n_pred1-1)*128:n_pred1*128,:].shape)
                                    ...
                                    pred_val_x[:,(m_pred1-1)*128:m_pred1*128, (n_pred1-1)*128:n_pred1*128,:] = preds_valx[0,:,:,:]
                            path_results = r"C:\Users\massi\Downloads\segmentation_models-master\images\complete_unsupervised_results_2020\\"
                            if not os.path.exists(path_results):
                                os.makedirs(path_results)
                            name_inputs1 = "B8_Test_" + name_inputs + "_" + date_3
