...
folder_1 = r"C:\Users\massi\Downloads\sentinel1 Salerno\train_data_\\" #r"D:\Albufera\\"#
...
for zone in range(1,2):#,6):
    ...
    for date_alb in range(1): 
        ...
        files_test = [r"C:\Users\massi\Downloads\sentinel1 Salerno\test_data" + str(zone) + "\\"] # "test_intradate_gamma.npz", "test_intradate_beta.npz"]
        dic_name_inputs = {r"C:\Users\massi\Downloads\sentinel1 Salerno\test_data" + str(zone) + "\\":"FiumePo_"}
        folder_trains = r"C:\Users\massi\Downloads\sentinel1 Salerno\test_data" + str(zone) + "\\" # 

        for file_test in files_test: 
            folder_train = folder_test #r"D:\Albufera_2019_processed\Sigma2\\"
            folder_test = folder_test # file_test #
    ...
    # print(dic)
                ...
                #             model.save_weights( name_inputs +combinations + "_" + BACKBONE + "_" + name_model + "_model_wIoU"+ str(size) +".h5")
            ############END TRAINING#############           
                        ...
                        if k_mod == "SVM" or k_mod == "RF" or k_mod == "GBC": 
                            ...
                            model.load_weights( name_inputs +combinations + "_" + BACKBONE + "_" + name_model + "_model_wIoU"+ str(size) +".h5")
        ...
        # #########TESTING ############################
                ...
                #                     chan = 6
                                chan = 2
                                X = np.ndarray(shape=(number_val,size,size,chan))
                                J = np.ndarray(shape=(size,size,chan))
                #                Y = np.ndarray(shape=(batch_size,size,size,1))
                                Y = np.ndarray(shape=(number_val,size,size,2))
                                Y1 = np.ndarray(shape=(number_val,size,size,2))
                                for k in range(0,number_val):
                                    i = ecc[k]
                                    H = np.load(os.path.join(folder_test, 'X_val_' + str(i) + '.npy'))  
                                    ...
                                    Y_H = np.load(os.path.join(folder_test, 'Y_val_' + str(i) + '.npy'))
                                    # print(np.max(Y_H))
                                    # print(Y_H.shape)
                                ...
                                name_inputs1 = r"C:\Users\massi\Downloads\segmentation_models-master\images\s1_results_fiumePo\\FiumePo" + str(zone) + "_" + name_inputs
                                for k in range(0,number_val): #x_test2.shape[0],x_test2.shape[0]//10): #x_test2.shape[0]):#x_test.shape[0],int(x_test.shape[0]/100)): 
                                    x_val_1 = x_test2[k,:,:,:]
                                    y_val_1 = y_test1[k,:,:,0]
                                    ...
                                    ndvi2 = pred_val_1.astype('float32')
                                    im2 =  name_inputs1 + combinations + "_" + BACKBONE +date_2  + '_' + name_model + '_output_'+ str(k) + '_wpatches'+ str(size) +'.tif'
                                    imsave(im2, ndvi2)
                                    ndvi3 = x_val_1[:,:,0].astype('float32')
                                    im3 = name_inputs1 + combinations + "_" + BACKBONE + date_2  +'_' + name_model + '_VV_'+ str(k) + '_wpatches'+ str(size) +'.tif'
                                    imsave(im3, ndvi3)
                                    ndvi4 = x_val_1[:,:,1].astype('float32')
                                    im4 =  name_inputs1 + combinations + "_" + BACKBONE +date_2  + '_' + name_model + '_VH_'+ str(k) + '_wpatches'+ str(size) +'.tif'
                                    imsave(im4, ndvi4)
                                    # y_val_1 = y_test1[k,:,:,1]
